...
export const BlockItem: React.FC<BlockItemProps> = ({ block, onClick, selectionIndex, disabled = false }) => {
    ...
    return (
        ...
        >
            {/* Selection Badge */}
            {isSelected && (
                <div className="absolute -top-3 -right-2 w-8 h-8 bg-orange-500 border-2 border-white text-white font-bold rounded-full flex items-center justify-center shadow-md z-10 animate-[pop_0.2s_ease-out]">
                    {selectionIndex}
                </div>
            )}

            <div className="flex items-center gap-3 w-full">
                <span className="text-2xl bg-black/10 rounded p-1 w-10 h-10 flex items-center justify-center shrink-0">
                    {block.icon}
                </span>
                <span className="font-bold tracking-wide leading-tight text-sm md:text-base">
                    {block.label}
                </span>
            </div>
        </div>
